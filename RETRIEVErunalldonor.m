function [responsevector,decision] = RETRIEVErunalldonor(theepitope)
...
%         status = movefile('Parameters.mat',['D' num2str(Daylimit) '_CULTURE_n' num2str(i) 'Parameters.mat']);
        load(['D' num2str(Daylimit) '_CULTURE_n' num2str(i) 'results.mat'],'AT_M_vector');
        response(Daylimit-4,i,1) = AT_M_vector(end,1);
...
%         status = movefile('Parameters.mat',['D' num2str(Daylimit) '_SAMPLE_n' num2str(i) 'Parameters.mat']);
        load(['D' num2str(Daylimit) '_SAMPLE_n' num2str(i) 'results.mat'],'AT_M_vector');
        response(Daylimit-4,i,2) = AT_M_vector(end,1);
    end
